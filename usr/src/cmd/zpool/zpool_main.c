...
get_usage(zpool_help_t idx)
{
	...
	case HELP_ATTACH:
		return (gettext("\tattach [-fw] <pool> <device> "
		    "<new-device>\n"));
	case HELP_CLEAR:
	...
	case HELP_INITIALIZE:
		return (gettext("\tinitialize [-csw] <pool> [<device> ...]\n"));
	case HELP_SCRUB:
...
/*
 * zpool replace [-fw] <pool> <device> <new_device>
 *
...
/*
 * zpool attach [-fw] <pool> <device> <new_device>
 *
 *	-f	Force attach, even if <new_device> appears to be in use.
 ...
 *	-p	Pause. Pauses in-progress scrub.
...
zpool_do_initialize(int argc, char **argv)
{
	...
	if (wait && (cmd_type != POOL_INITIALIZE_DO)) {
...
print_wait_status_row(wait_data_t *wd, zpool_handle_t *zhp)
{
	...
	if (pss != NULL && pss->pss_state == DSS_SCANNING &&
		...
		int64_t rem = pss->pss_to_examine - pss->pss_examined;
...
wait_status_thread(void *arg)
{
	...
	for (;;) {
		...
		if (zpool_refresh_stats(zhp, &missing) != 0 || missing ||
			...
			zpool_close(zhp);
			return (void *)(missing ? 0 : 1);
		}

		print_wait_status_row(wd, zhp);

		timeout.tv_sec += wd->wd_interval;
