...
typedef struct sublivelist_verify {
	...
	btree_t sv_all_allocs;

	/* all FREE'd blkptr_t in one sub-livelist */
	btree_t sv_all_frees;
    ...
    dmu_tx_t *tx)
{
	...
	btree_index_t where;
	if (free) {
		btree_add(&sv->sv_pair, bp);
		/* Check if the FREE is a duplicate */
		if (btree_find(&sv->sv_all_frees, bp, &where) != NULL) {
			snprintf_blkptr_compact(blkbuf, sizeof (blkbuf), bp,
			    free);
			(void) printf("\tERROR: Duplicate FREE: %s\n", blkbuf);
		} else {
			btree_insert(&sv->sv_all_frees, bp, &where);
		}
	} else {
		/* Check if the ALLOC has been freed */
		if (btree_find(&sv->sv_pair, bp, &where) != NULL) {
			btree_remove_from(&sv->sv_pair, &where);
		} else {
			for (int i = 0; i < SPA_DVAS_PER_BP; i++) {
				...
				if (btree_find(&sv->sv_leftover, &svb,
				    &where) == NULL) {
					btree_insert(&sv->sv_leftover, &svb,
					    &where);
				}
			}
		}
		/* Check if the ALLOC is a duplicate */
		if (btree_find(&sv->sv_all_allocs, bp, &where) != NULL) {
			snprintf_blkptr_compact(blkbuf, sizeof (blkbuf), bp,
			    free);
			(void) printf("\tERROR: Duplicate ALLOC: %s\n", blkbuf);
		} else {
			btree_insert(&sv->sv_all_allocs, bp, &where);
		}
	}
	return (0);
}
...
sublivelist_verify_func(void *args, dsl_deadlist_entry_t *dle)
{
	...
	btree_create(&sv->sv_all_allocs, livelist_compare,
	    sizeof (blkptr_t));

	btree_create(&sv->sv_all_frees, livelist_compare,
	    sizeof (blkptr_t));

	btree_create(&sv->sv_pair, livelist_compare,
	    sizeof (blkptr_t));

	err = bpobj_iterate_nofree(&dle->dle_bpobj, sublivelist_verify_blkptr,
	    sv, NULL);

	btree_clear(&sv->sv_all_allocs);
	btree_destroy(&sv->sv_all_allocs);

	btree_clear(&sv->sv_all_frees);
	btree_destroy(&sv->sv_all_frees);

	blkptr_t *e;
	btree_index_t *cookie = NULL;
	while ((e = btree_destroy_nodes(&sv->sv_pair, &cookie)) != NULL) {
		snprintf_blkptr_compact(blkbuf, sizeof (blkbuf), e, B_TRUE);
		(void) printf("\tERROR: Unmatched FREE: %s\n", blkbuf);
	}
	btree_destroy(&sv->sv_pair);
...
sublivelist_verify_lightweight(void *args, dsl_deadlist_entry_t *dle)
{
	...
	btree_create(&sv.sv_leftover, livelist_block_compare,
	    sizeof (sublivelist_verify_block_t));
	int err = sublivelist_verify_func(&sv, dle);
	btree_clear(&sv.sv_leftover);
	btree_destroy(&sv.sv_leftover);
	return (err);
}
    ...
    uint64_t offset, uint64_t size)
{
	...
	sublivelist_verify_block_t *found =
	    btree_find(&mv->mv_livelist_allocs, &svb, &where);
	if (found == NULL) {
		found = btree_next(&mv->mv_livelist_allocs, &where, &where);
	}
...
mv_populate_livelist_allocs(metaslab_verify_t *mv, sublivelist_verify_t *sv)
{
	btree_index_t where;
	sublivelist_verify_block_t *svb;
	ASSERT3U(btree_numnodes(&mv->mv_livelist_allocs), ==, 0);
	for (svb = btree_first(&sv->sv_leftover, &where);
	    svb != NULL;
	    svb = btree_next(&sv->sv_leftover, &where, &where)) {
		...
		btree_add(&mv->mv_livelist_allocs, svb);
	}

	for (svb = btree_first(&mv->mv_livelist_allocs, &where);
	    svb != NULL;
	    svb = btree_next(&mv->mv_livelist_allocs, &where, &where)) {
		btree_remove(&sv->sv_leftover, svb);
	}
}
...
livelist_metaslab_validate(spa_t *spa)
{
	...
	for (uint64_t c = 0; c < rvd->vdev_children; c++) {
		...
		for (uint64_t mid = 0; mid < vd->vdev_ms_count; mid++) {
			...
			btree_create(&mv.mv_livelist_allocs,
			...
			btree_clear(&mv.mv_livelist_allocs);
			btree_destroy(&mv.mv_livelist_allocs);
		}
	}
	 ...
	 */
	if (btree_numnodes(&sv.sv_leftover) == 0) {
		btree_destroy(&sv.sv_leftover);
		return;
	}
	...
	btree_index_t *where = NULL;
	sublivelist_verify_block_t *svb;
	while ((svb = btree_destroy_nodes(&sv.sv_leftover, &where)) != NULL) {
...
{
	(void) fprintf(stderr,
	    "Usage:\t%s [-AbcCdDFGhikLmMPsvXy] [-e [-V] [-p <path> ...]] "
	    "[-I <inflight I/Os>]\n"
...
print_vdev_metaslab_header(vdev_t *vd)
{
	...
	(void) printf("\tvdev %10llu", (u_longlong_t)vd->vdev_id);
...
dump_log_spacemaps(spa_t *spa)
{
	...
	for (spa_log_sm_t *sls = avl_first(&spa->spa_sm_logs_by_txg);
		...
		(void) printf("Log Spacemap object %llu txg %llu\n",
		    (u_longlong_t)sls->sls_sm_obj,
		    (u_longlong_t)sls->sls_txg);
...
/* ARGSUSED */
static int
load_unflushed_svr_segs_cb(spa_t *spa, space_map_entry_t *sme, uint64_t txg,
    void *arg)
{
...
static int
count_unflushed_space_cb(spa_t *spa, space_map_entry_t *sme, uint64_t txg,
    void *arg)
{
...
zdb_leak_init(spa_t *spa, zdb_cb_t *zcb)
{
	...
	zcb->zcb_vd_obsolete_counts =
	    umem_zalloc(rvd->vdev_children * sizeof (uint32_t *), UMEM_NOFAIL);
...
dump_block_stats(spa_t *spa)
{
	 ...
	 * the pool is perfectly consistent, the space maps will be empty
	...
	for (uint64_t v = 0; v < spa->spa_root_vdev->vdev_children; v++) {
		...
		char mem[32];
		zdb_nicenum(vdev_indirect_mapping_size(vim), mem, sizeof (mem));
...
mos_leak_vdev_top_zap(vdev_t *vd)
{
	...
	int error = zap_lookup(spa_meta_objset(vd->vdev_spa), vd->vdev_top_zap,
	    VDEV_TOP_ZAP_MS_UNFLUSHED_PHYS_TXGS, sizeof (ms_flush_data_obj), 1,
	    &ms_flush_data_obj);
	if (error == ENOENT)
		return;
	ASSERT0(error);
...
mos_leak_vdev(vdev_t *vd)
{
	...
	for (unsigned int c = 0; c < vd->vdev_children; c++) {
...
mos_leak_log_spacemaps(spa_t *spa)
{
	...
	int error = zap_lookup(spa_meta_objset(spa), DMU_POOL_DIRECTORY_OBJECT,
...
static int
log_spacemap_obsolete_stats_cb(spa_t *spa, space_map_entry_t *sme, uint64_t txg,
    void *arg)
{
