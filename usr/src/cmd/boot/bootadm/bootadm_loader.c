...
print_menu_cb(ofmt_arg_t *ofarg, char *buf, uint_t bufsize)
{
	...
	case 3:
		...
		break;
	case 4:
		if (entry->me_nextboot == B_TRUE)
			(void) snprintf(buf, bufsize, "   T");
		else if (entry->me_active == B_TRUE)
...
static boolean_t
menu_next_active(be_node_list_t *be_nodes, const char *bootfs)
{
	be_node_list_t *be_node;
	boolean_t rv = B_FALSE;
...
menu_read(struct menu_lst *menu, char *menu_path)
{
	...
	do {
		...
		mp->me_nextboot = menu_next_active(be_nodes, bootfs);
