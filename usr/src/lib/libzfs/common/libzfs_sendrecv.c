...
send_prelim_records(zfs_handle_t *zhp, const char *from, int fd,
    ...
    boolean_t dryrun, nvlist_t **fssp, avl_tree_t **fsavlp)
{
	int err = 0;
	char *packbuf = NULL;
	size_t buflen = 0;
	...
	if (gather_props) {
		...
		if ((err = gather_nvlist(zhp->zfs_hdl, tofs,
		    from, tosnap, recursive, verbose, &fss, fsavlp)) != 0) {
    ...
    void *cb_arg, nvlist_t **debugnvp)
{
	...
	if (flags->replicate || flags->doall || flags->props) {
		...
		(void) snprintf(full_tosnap_name, sizeof (full_tosnap_name),
		...
		err = send_prelim_records(tosnap, fromsnap, outfd,
		    ...
		    flags->verbosity > 0, flags->dryrun, &fss, &fsavl);
		zfs_close(tosnap);
		if (err != 0)
			goto err_out;
	}
