...
dsl_dir_remove_livelist(dsl_dir_t *dd, dmu_tx_t *tx, boolean_t total)
{
	...
	if (ll_condense_thread != NULL && zthr_isrunning(ll_condense_thread) &&
	    (to_condense.ds != NULL) && (to_condense.ds->ds_dir == dd)) {
			...
			VERIFY0(zthr_wait(ll_condense_thread));
			/*
			 * If we've returned from zthr_wait without clearing the
			 * to_condense data structure it's either because the
			 * no-wait synctask has started (which is indicated by
			 * 'syncing' field of to_condense) and we can expect it
			 * to clear to_condense on its own. Otherwise, we
