...
zio_dva_allocate(zio_t *zio)
{
	...
	/*
	 * Try allocating the block in the normal metaslab class.
	 * If that's full, allocate it in slog space,
	 * and if both are full, allocate as a gang block.
	 */
	...
	if (error == ENOSPC && !spa_has_log_device(spa) &&
		...
		if (zfs_flags & ZFS_DEBUG_METASLAB_ALLOC) {
			zfs_dbgmsg("%s: metaslab allocation failure, "
			    "trying log class: zio %p, size %llu, error %d",
	...
	if (error == ENOSPC && zio->io_size > SPA_MINBLOCKSIZE) {
		if (zfs_flags & ZFS_DEBUG_METASLAB_ALLOC) {
			zfs_dbgmsg("%s: metaslab allocation failure, "
			    "trying ganging: zio %p, size %llu, error %d",
			    spa_name(spa), zio, zio->io_size, error);
		}
