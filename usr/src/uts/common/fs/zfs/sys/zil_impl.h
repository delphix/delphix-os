 ...
 * transitioning from "closed" to "opened" the zilog's "zl_writer_lock"
 * must be held.
 *
 * After the lwb is "opened", it can transition into the "issued" state
 * via zil_lwb_write_issue(). Again, the zilog's "zl_writer_lock" must
 ...
 * held, *not* the "zl_writer_lock".
 *
 * The zilog's "zl_writer_lock" can become heavily contended in certain
 ...
 * having to acquire the "zl_writer_lock" for each lwb ZIO completion,
 ...
 * "zl_writer_lock" will prevent a concurrent thread from transitioning
