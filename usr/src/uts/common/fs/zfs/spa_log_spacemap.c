 ...
 * once.
 */
uint64_t zfs_log_sm_blksz = 1ULL << 17;
 ...
 */
uint64_t zfs_unflushed_max_mem_ppm = 1000;

/*
 * Specific hard-limit in memory that ZFS allows to be used for
 * unflushed changes.
 */
uint64_t zfs_unflushed_max_mem_amt = 1ULL << 30;
 ...
 */
uint64_t zfs_unflushed_log_block_pct = 400;
 ...
 */
uint64_t zfs_unflushed_log_block_min = 1000;
 ...
 * terms of blocks.
 */
uint64_t zfs_unflushed_log_block_max = (1ULL << 18);
 ...
 */
uint64_t zfs_max_logsm_summary_length = 10;
 ...
 */
uint64_t zfs_min_metaslabs_to_flush = 1;
...
spa_estimate_incoming_log_blocks(spa_t *spa)
{
	...
	return ((steps > 0) ? DIV_ROUNDUP(sum, steps) : 0);
}
...
{
	uint64_t blocks_per_row = MAX(1, DIV_ROUNDUP(spa_log_sm_blocklimit(spa),
...
spa_estimate_metaslabs_to_flush(spa_t *spa)
{
	...
	for (log_summary_entry_t *e = list_head(&spa->spa_log_summary);
		...
		max_flushes_pertxg = MAX(max_flushes_pertxg,
		    DIV_ROUNDUP(total_flushes, txgs_in_future));
...
spa_ld_log_sm_metadata(spa_t *spa)
{
	...
	if (error == ENOENT) {
		...
		zfs_dbgmsg("spa_ld_log_sm_metadata(%s): failed at zap_lookup "
		    "[error=%d]", spa->spa_name, error);
		return (error);
	}
...
spa_ld_log_sm_data(spa_t *spa)
{
	...
	for (spa_log_sm_t *sls = avl_first(&spa->spa_sm_logs_by_txg);
	    sls; sls = AVL_NEXT(&spa->spa_sm_logs_by_txg, sls)) {
		space_map_t *sm = NULL;
		int error = space_map_open(&sm, spa_meta_objset(spa),
	...
	/*
	 * Now that all the metaslabs contain their unflushed changes:
